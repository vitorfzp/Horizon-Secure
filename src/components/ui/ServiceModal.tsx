      ...
      <div ref={modalRef} className={`relative bg-[#0A0A0A] border border-white/10 w-full max-w-3xl rounded-2xl overflow-hidden flex flex-col max-h-[85vh] transform transition-all duration-500 tech-border-glow shadow-[0_0_50px_rgba(99,102,241,0.2)] ${isVisible ? 'scale-100 opacity-100 translate-y-0' : 'scale-95 opacity-0 translate-y-8'}`}>
        ...
        <div className="p-8 md:p-10 overflow-y-auto custom-scrollbar">
            ...
            <div>
              ...
              <div className="flex gap-2 flex-wrap">
                {service.benefits.map((benefit: string, i: number) => (
                  <span key={i} className={`text-xs font-mono uppercase tracking-wider font-bold px-3 py-1.5 rounded-md bg-${service.color}-500/10 text-${service.color}-300 border border-${service.color}-500/20`}>
                    {benefit}
                  </span>
                ))}
              </div>
          ...
          <div className="flex flex-col sm:flex-row justify-end gap-4 pt-6 border-t border-white/10">
             ...
             <Link href="/contato">
                <Button variant="primary" icon={ArrowRight}>Falar com Especialista</Button>
             </Link>
          </div>
        </div>
      </div>
